  ...
  useEffect(() => {
    const fetchTaskData = async () => {
      try {
        ...
        const taskUrl = `${base}/tasks/${taskId}?workspaceId=${workspaceId}&userId=${userId}${modeQuery}`;
        const updatesUrl = `${base}/tasks/${taskId}/updates?workspaceId=${workspaceId}&userId=${userId}${modeQuery}`;
        const subtasksUrl = `${base}/tasks/${taskId}/subtasks?workspaceId=${workspaceId}&userId=${userId}${modeQuery}`;
        const commentsUrl = `${base}/tasks/${taskId}/comments?workspaceId=${workspaceId}&userId=${userId}${modeQuery}`;

        const [taskResponse, updatesResponse, subtasksResponse, commentsResponse] =
          await Promise.all([
            fetch(taskUrl),
            fetch(updatesUrl),
            fetch(subtasksUrl),
